"""app/message/dedupe.py — Writer 持久化去重窗口（Redis，fail-open）。

实现设计 §3.1、C-MESSAGE-WRITE-2。与 access/dedupe.py 完全同构，
唯一差异是键前缀和窗口单位（`dedup_window_seconds`，下界 = kafka_retention_seconds）。
"""
...
KEY_PREFIX: Final = "amp:message:dedupe:"  # 键 = amp:message:dedupe:{log_id}
...
async def filter_unseen(redis: Redis, log_ids: list[str]) -> tuple[set[str], bool]:
    ...
    fail-open：Redis 异常 → 返回 (全部 log_ids, False)。
    """
    if not log_ids:
        return set(), True
...
async def mark_seen(redis: Redis, log_ids: list[str], *, ttl_seconds: int) -> None:
    ...
    fail-open：异常只告警不抛（标记缺失只致后续可能重复，不丢数）。
    """
    if not log_ids:
        return
