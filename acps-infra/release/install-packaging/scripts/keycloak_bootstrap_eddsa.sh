...
kcadm() {
    ...
    elif [[ -n "${KEYCLOAK_CONTAINER}" ]]; then
        docker exec "${KEYCLOAK_CONTAINER}" /opt/keycloak/bin/kcadm.sh "$@"
    else
        log "KEYCLOAK_HOME or KEYCLOAK_CONTAINER is required"
        return 1
    fi
}
...
web_base_url_for_realm() {
    ...
    case "${realm}" in
        ...
        acps-monitor)
            ...
            ;;
        acps-leader)
            echo "${LEADER_WEB_BASE_URL:-http://localhost:${LEADER_WEB_PORT:-9030}}"
            ;;
        *)
...
# business.yml Step C 群组轮询最长可达 600s，且中间多次 LLM；默认 300s access token 会在 C 中途过期。
# 幂等抬到 1800s（与验收机手工修复一致）；不缩短已更长的现场配置。
get_access_token_lifespan() {
    local realm="$1"
    kcadm get "realms/${realm}" | python3 -c '
import json
import sys

value = json.load(sys.stdin).get("accessTokenLifespan")
if value is None:
    print("")
else:
    print(int(value))
...
ensure_access_token_lifespan() {
    local realm="$1"
    local want="${KEYCLOAK_ACCESS_TOKEN_LIFESPAN_SECONDS:-1800}"
    local current

    current="$(get_access_token_lifespan "${realm}")"
    if [[ -n "${current}" && "${current}" =~ ^[0-9]+$ && "${current}" -ge "${want}" ]]; then
        log "${realm} accessTokenLifespan=${current} (>= ${want})"
        return 0
    fi

    log "set ${realm} accessTokenLifespan=${want} (was ${current:-unset})"
    kcadm update "realms/${realm}" -s "accessTokenLifespan=${want}"

    current="$(get_access_token_lifespan "${realm}")"
    if [[ -z "${current}" || ! "${current}" =~ ^[0-9]+$ || "${current}" -lt "${want}" ]]; then
        log "failed to set ${realm} accessTokenLifespan to ${want} (still ${current:-unset})"
        return 1
    fi
    log "${realm} accessTokenLifespan verified=${current}"
}
...
# Lifespan before EdDSA/scope work: later steps may fail mid-realm; Step C needs >=1800s on all realms.
for realm in "${REALMS[@]}"; do
    ensure_access_token_lifespan "${realm}"
done
